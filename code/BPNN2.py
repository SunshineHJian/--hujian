...
if __name__ == '__main__':
    ...
    # plot函数作图
    plt.plot(X, y, color="r", linestyle="--", marker="*", linewidth=0.5, label='original values')
    plt.plot(X_pre, y_pre, color="g", linestyle="--", marker="*", linewidth=0.5, label='polyfit values')

    plt.xlabel('time/month')
    plt.ylabel('scores/*100%')
    plt.legend(loc=2)  # 指定legend的位置右下角
    plt.title('polyfitting')
    plt.savefig('image/pre_micro.png')
    plt.show()
